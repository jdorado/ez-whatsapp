...
import { chmod, unlink, readFile } from 'node:fs/promises';
...
export async function serve(dir, createTransport, socketPath = join(dir, 'service.sock'), options = {}) {
  const store = new Store(dir); await store.init();
  store.qrPath = options.qrPath;
  ...
  try {
    ...
    if (!options.singleAccount) {
      accounts = new Accounts(store, service, socketPath, (profile, socket, qrPath) => serve(profile, createTransport, socket, { singleAccount: true, qrPath, accountName: basename(profile) }));
      await accounts.init();
    }
    server = createServer(async (req, res) => {
      ...
      try {
        ...
        const data = await (accounts ?? service).call(input.command, input.args);
    ...
    return { service, store, close: async () => {
      ...
      try { await accounts?.close(); } finally {
        try { await transport.close(); await store.queue; } finally { await release(); }
      }
    } };
  } catch (e) {
    server?.close();
    try { await accounts?.close(); } finally {
      try { await transport?.close(); } finally { await release(); }
    }
    throw e;
  }
}
